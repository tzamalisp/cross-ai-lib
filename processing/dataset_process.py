...
from processing.motion.features_extraction import motion_features_extraction
...
class Dataset:
    def __init__(self):
        ...
        self.test_X = None
        self.test_y = None
        self.test_y_vec = None  # Test_y as a vector (used in classification report)
        self.uuid = None
        self.shape = None
        self.number_of_instances = None
        self.number_of_classes = None
        self.descriptions = None  # User friendly descriptions of labels (used in the confusion-matrix)
        self.input_instance_length = None
        self.labels_descriptions = None

    def set_data(self, X, y):
        """
        Used when the dataset is passed by reference and is not imported from filesystem
        Args:
    ...
    def load_dataset_by_uuid(self, uuid):
        ...
        Returns: None
            ndarray of the dataset that is stored in the disk
        """
        path_to_dataset = self.path_to_dataset_directory.joinpath(uuid + ".npz")
        path_to_datalen_txt = self.path_to_dataset_directory.joinpath(uuid + ".txt")
        self.uuid = uuid
    ...
    def labels_to_timesteps(self):
        ...
        This method can be used ONLY when the data have occured with rolling window.
        :return: None
            Transforms self.y
        """
        # rolling_window_size = self.shape[0]
        self.y = self.y[:, np.newaxis] * np.ones((self.number_of_instances, self.input_instance_length))
    ...
    def last_n_percent(self, n=0.2):
        ...
        Currently not used.
        Args:
            n (float): Percent as float (range 0 to 1) of the dataset values to return.
    ...
    def load_labels_descriptions(self, dataset_labels_dict):
        """From a given dict_label, it genberates the description for each class.
        It is expected that the values in y have occured from the given dict_label
        that should originate from the project configuration file
...
def save_dataset(saveid, train_X, test_X, train_y, test_y, path_to_save=None):
    ...
    if path_to_save is None:
        path_to_save = Path(project_configuration["project_store_path"]).joinpath("datasets").joinpath(filename)
...
def load_dataset_npz(preprocessing_uuid, path_to_datasets_dir=None):
    """
    Loads the dataset from the default project dataset directory. The dataset should be
    in NPZ format and contain the keys train_X, test_X, train_y, test_y.
    Args:
        preprocessing_uuid (str): The uuid of the preprocessing task, according to which the dataset
            was generated.
        path_to_datasets_dir(pathlib.Path or str, optional): Path to the datasets directory
    Returns:
    ...
    if path_to_data.is_file():
        ...
        msg = "{} does not exist!". \
            format(path_to_data.relative_to(project_configuration["project_store_path"]))
        logging.error(msg)
        raise Exception(msg)
    return data
...
def dataset_postprocessing(train_X, test_X, configuration):
    if configuration.get("scaling"):
        ...
        logging.warning("No scaling option found in configuration. Dataset will not be scaled!")
    if configuration.get("dimensionality_reduction") is not None:
        # TODO review configuration template for dimensionality reduction.
        decomposer, dimensions, train_X, test_X = decompose(train_data=train_X,
...
def dataset_features_extraction(dfs_list, dfs_labels, features_categories, dataset_domain,
                                features_extraction_parameters=None):
    """
    Calculates features according to the given categories from a list of DataFrames that have occurred from
    a motion dataset.
    Args:
        dfs_list (list):
        dfs_labels (numpy array):
        features_categories (list):
        dataset_domain (str): Description of the dataset category (motion, signal, timeseries, text, audio)
        features_extraction_parameters (dict):
    Returns:
        df_feats (pandas DataFrame): A dataframe that holds the selected features or categories of features
        and the label of the instance.
    """
    if features_extraction_parameters is None:
        features_extraction_parameters = project_configuration["processing"].get("features_extraction_parameters")
        if features_extraction_parameters is None:
            msg = "Attempting features extraction but no parameters have been defined in configuration."
            logging.error(msg)
            raise Exception(msg)
    ...
    for df, label in tqdm(zip(dfs_list, dfs_labels)):
        ...
        if df.shape[0] != 0:
            if dataset_domain == "motion":
                feats = motion_features_extraction(df, feature_categories=features_categories, sampling_frequency=100,
                                                   features_extraction_parameters=features_extraction_parameters)
            else:
                msg = "{} feature extraction is not yet implemented or supported.".format(dataset_domain)
                logging.error(msg)
                raise Exception(msg)
...
def pca_x_components_analysis(data, components, labels, path_to_save, **kwargs):
    ...
    logging.info("Data shape after transformation : {}".format(pca_x_data.shape))
    if components == 2:
        plot_scatter([pca_x_data[:, 0], pca_x_data[:, 1]],
                     xlabel="First principal component",
                     ylabel="Second Principal Component",
                     c=labels,
                     path_to_save=path_to_save.joinpath("pca_{}_components.png".format(components_str)))
    elif components == 3:
    ...
    logging.info("PCA - {} components explained variance : {}".format(components_str, pca_x.explained_variance_))
    logging.info(
        "PCA - {} components explained variance ratio : {}".format(components_str, pca_x.explained_variance_ratio_))
    logging.info("")
    logging.info("Length of each component (It should be the same as the features extracted):")
    for i in range(0, pca_x.n_components_):
        logging.info('Component {} length: {}'.format((i + 1), len(abs(pca_x.components_[i]))))
    logging.info("PCA - {0} - Noise variance : {1:.4f}".format(components_str, pca_x.noise_variance_))
    columns = kwargs.get("columns", None)
    pca_x_components = pd.DataFrame(pca_x.components_, columns=columns)
    plot_dataframe_heatmap(data=pca_x_components,
                           path_to_save=path_to_save.joinpath("pca_{}_components_data_frame.png".
                                                              format(components_str)))
    logging.info("")
    logging.info("Component maximum / minimum values")
    for i in range(0, pca_x.n_components_):
        logging.info('Component {} maximum value {}'.format(i + 1, pca_x_components.iloc[i].max()))
        logging.info('Component {} minimum value {}'.format(i + 1, pca_x_components.iloc[i].min()))
    pca_x_components_abs = pd.DataFrame(abs(pca_x.components_), columns=columns)
    plot_dataframe_heatmap(data=pca_x_components,
                           path_to_save=path_to_save.joinpath("pca_{}_components_data_frame_abs_values.png".
                                                              format(components_str)))
    logging.info("")
    logging.info("Component maximum / minimum values")
    for i in range(0, pca_x.n_components_):
        logging.info('Component {} maximum absolute value {}'.format(i + 1, pca_x_components_abs.iloc[i].max()))
        logging.info('Component {} minimum absolute value {}'.format(i + 1, pca_x_components_abs.iloc[i].min()))
...
def components_analysis(dataset_X, dataset_y, path_to_save):
    ...
    logging.info("Ploting the relation between the Variance and the Dimensions")
    ...
    pca_x_components_analysis(x_train_scaler, 2, train_y, path_to_save, columns=dataset_X.columns)
    pca_x_components_analysis(x_train_scaler, 3, train_y, path_to_save, columns=dataset_X.columns)
    pca_x_components_analysis(x_train_scaler, 0.95, train_y, path_to_save, columns=dataset_X.columns)
