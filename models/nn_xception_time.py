...
def xception_time(number_of_classes=None, train_data_shape=None, xception_adaptive_size=50,
    ...
    if train_data_shape[0] % xception_adaptive_size != 0:
        xception_adaptive_size = int(train_data_shape[0] / xception_adapt_ws_divide)
    else:
    ...
    # Dropout
    x = dropout_layer(input_tensor=x, drp_on=True, drp_rate=drp_mid, spatial=spatial)

    # stack 3 Conv1x1 Convolutions to reduce the time-series to the number of the classes
    x_post = conv1d_block(input_tensor=x, nf=head_nf/2, drp_on=False, drp_rate=0.5, spatial=True,
                          kernel_initialize=kernel_initialize,
                          kernel_regularize=kernel_regularize,
                          kernel_constraint=kernel_constraint)
    x_post = conv1d_block(input_tensor=x_post, nf=head_nf/4, drp_on=False,  drp_rate=0.5, spatial=True,
                          kernel_initialize=kernel_initialize,
                          kernel_regularize=kernel_regularize,
                          kernel_constraint=kernel_constraint)
    x_post = conv1d_block(input_tensor=x_post, nf=number_of_classes, drp_on=False, drp_rate=0.5, spatial=True,
                          kernel_initialize=kernel_initialize,
                          kernel_regularize=kernel_regularize,
                          kernel_constraint=kernel_constraint)
    ...
    # Dropout
    aap = dropout_layer(input_tensor=aap, drp_on=True, drp_rate=drp_high, spatial=spatial)
...
def conv1d_block(input_tensor, nf, ks=1, strd=1, pad="same", bias=False, bn=True, act=True, act_func="relu",
...
def xception_block(input_tensor, n_filters, depth=4, use_residual=True, kernel_initialize=None, kernel_regularize=None,
                   kernel_constraint=None):
    """
...
def _xception_module(input_tensor, n_filters, use_bottleneck=True, kernel_size=41, stride=1,
    ...
    if use_bottleneck and n_filters > 1:
        ...
        input_inception = Conv1D(filters=n_filters, kernel_size=1, padding="valid",
                                 use_bias=False,
                                 kernel_initializer=kernel_initialize,
                                 kernel_regularizer=kernel_regularize,
                                 kernel_constraint=kernel_constraint
    ...
    # SECOND PATH
    x = MaxPooling1D(pool_size=3, strides=stride, padding="same")(input_tensor)

    x = Conv1D(filters=n_filters, kernel_size=1, padding="valid", use_bias=False,
               kernel_initializer=kernel_initialize,
               kernel_regularizer=kernel_regularize,
               kernel_constraint=kernel_constraint
               )(x)
...
def _shortcut_layer(input_tensor, out_tensor, n_filters, kernel_initialize, kernel_regularize,
                    kernel_constraint):
    """
