...
from text_processing.preprocessing import strip_urls_entities_emojis,\
    strip_emojis
from text_processing.preprocessing import clean_up_punctuation_main, \
    strip_accents
from text_processing.preprocessing import clean_up_digits_main, \
    tokenizing
# TTR
...
def entropy(string):
    ...
    prob = [float(string.count(c)) / len(string) for c
            in dict.fromkeys(list(string))]
...
def average_word_length_sentence(text):
    total_length = sum(len(strip_urls_entities_emojis(word)) for sentence in
                       text for word in sentence.split())
...
def avg_chars_per_sentence(clean_text, nltk_language="greek"):
    ...
    for sentence in sentences:
        ...
        sentence_clean = " ".join(clean_up_punctuation_main(sentence,
                                                            mode="word"))
...
def count_vowels_consonants(clean_text, vowels="αεηιυοω",
                            consonants="βγδζθκλμνξπρστφχψ"):
    """
...
def count_consecutive_chars(clean_text, vowels="αεηιυοωaeiou", mode="tweet"):
    """
    Counts consecutive characters found in a string. These are,
     simple characters, consecutive vowels, and consecutive
    found consonants.
    Args:
        clean_text: (str) The clean text without the entities.
        vowels: (str) The vowels to be excluded or included to the counting.
        mode: The tokenizer mode. Default: "tweet". Other tokenizers:
         "tweet_extend", "word"
    ...
    for item in clean_text_list:
        consonants_list.append("".join(re.findall(r"[^{}]+".format(vowels),
                                                  item.lower())))
        vowels_list.append("".join(re.findall(r"[{}]+".format(vowels),
                                              item.lower())))

    consonants_lengths = [len(word) for word in consonants_list]
    vowels_list = [len(word) for word in vowels_list]

    # return the max number of identical repetitions found in the cleaned
    # text greater than a given value.
...
def nlp_post_processing(clean_text):
    ...
    for token in doc:
        ...
        #       .format(token, token.tag_, token.pos_
    ...
    nlp_post_process_dict["sentiment_subjectivity"] = \
        doc._.sentiment.subjectivity

    return nlp_post_process_dict


def count_entities(text, tokenizer=TweetTokenizer(strip_handles=False,
                                                  reduce_len=False)):
    """
    ...
    entities = [tok for tok in tokens if tok.startswith(('#', '@', 'http',
                                                         'https'))]
    return len(entities)
...
def repetition_identical_cons_chars(clean_text, repetition=3):
    """
    Counts the number of repetitions of an identical char that it is found in
     a string greater than a value.

    Args:
        clean_text: (str) The cleaned text from urls, entities, punctuations,
        and digits.
