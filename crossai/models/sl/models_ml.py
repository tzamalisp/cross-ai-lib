...
class MLClassifier:
    ...
    def load(self):
        path_to_models = Path(project_configuration.get(
            "project_store_path")).joinpath("models")
        models_paths_list = [i for i in path_to_models.glob(
            "*{}*".format(self.model_name))]
        if not models_paths_list:
            msg = "Model {} does not exist in models directory".format(
                self.model_name)
            logging.error(msg)
            raise Exception(msg)
