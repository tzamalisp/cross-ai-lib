...
def model_arguments_check_transform(config):
    ...
    # DATA RELATED
    # raise exception in case the number of classes or the train data shape
    # is not defined to the classifier params
    if config["clf_params"]["number_of_classes"] is None:
        print("Please fill the number of classes parameter to the config as"
              " config['clf_params']['number_of_classes']")
        raise Exception
    if config["clf_params"]["train_data_shape"] is None:
        print("Please fill the training data shape to the config as"
              " config['clf_params']['train_data_shape']")
        raise Exception

    # KERNEL
    # ensure the kernel initializer is of valid form and inside the
    # list of accepted kernel initializers
    for initializer in config["param_grid_train"]["kernel_initialize"]:
        if initializer not in kernel_initializers_valid_list:
            ...
            raise Exception
    # ensure the kernel regularize is a float value if not None,
    # and create the L2 Regularization
    for idx, regularize in enumerate(config["param_grid_train"]
                                     ["kernel_regularize"]):
        if regularize is not None:
            config["param_grid_train"]["kernel_regularize"][idx] = \
                float(regularize)
    # ensure the kernel constraint is an integer value if not None,
    # and create the MaxNorm constraint
    for idx, constraint in enumerate(config["param_grid_train"]
                                     ["kernel_constraint"]):
        if constraint is not None:
            config["param_grid_train"]["kernel_constraint"][idx] = \
                int(constraint)
    ...
    for idx, epsilon in enumerate(config["param_grid_train"]["adam_epsilon"]):
        if "adam" in config["param_grid_train"]["optimizer"] and epsilon is\
                not None:
