...
def inception_time(number_of_classes=None, train_data_shape=None,
    ...
    for d in range(depth):
        ...
        x = _inception_module(input_tensor=x, use_bottleneck=use_bottleneck,
        ...
        if use_residual and d % 3 == 2:
            x = _shortcut_layer(input_tensor=input_res, out_tensor=x,
                                kernel_initialize=kernel_initialize,
                                kernel_regularize=kernel_regularize,
                                kernel_constraint=kernel_constraint)
            input_res = x
...
def _shortcut_layer(input_tensor, out_tensor, kernel_initialize,
...
def _inception_module(input_tensor, use_bottleneck,
                      bottleneck_size, activation, nb_filters, kernel_size,
    ...
    for i in range(len(kernel_size_s)):
        print("Inception filters: {} - kernel: {}".format(nb_filters,
                                                          kernel_size_s[i]))
        conv_list.append(Conv1D(filters=nb_filters,
    ...
    max_pool_1 = MaxPooling1D(pool_size=3, strides=stride,
