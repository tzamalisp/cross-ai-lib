...
def callback_lr_scheduler(config):
    """
    Learning Rate Scheduling: Calls the appropriate functionality or callback for the predefined scheduling task:
    ...
    if config["clb_lr_scheduler"]["mode"] == "exponential":
        scheduler_exp = exponential_decay(lr0=config["clb_lr_scheduler"]["exponential"]["lr0"],
                                          s=config["clb_lr_scheduler"]["exponential"]["steps"])
        clb = LearningRateScheduler(schedule=scheduler_exp, verbose=config["fit_params"]["verbose"])
    elif config["clb_lr_scheduler"]["mode"] == "piecewise":
        scheduler_pws = piecewise_constant(boundaries=config["clb_lr_scheduler"]["piecewise"]["boundaries"],
                                           values=config["clb_lr_scheduler"]["piecewise"]["values"])
        clb = LearningRateScheduler(schedule=scheduler_pws, verbose=config["fit_params"]["verbose"])
    elif config["clb_lr_scheduler"]["mode"] == "performance":
        clb = ReduceLROnPlateau(monitor=config["clb_lr_scheduler"]["clb_plateau"]["monitor"],
