...
def callbacks_list(config, early_stop=True, lr_scheduler=True, checkpoint=True, tensorboard=True):
    """

    Args:
        config: The configuration data.
        early_stop: Define if adding the Early Stopping callback to the callbacks list.
        lr_scheduler: Define if adding the corresponding Learning Rate Scheduling callback to the callbacks list.
        checkpoint: Define if adding the Model Checkpoint callback to the callbacks list.
        tensorboard: Define if adding the Tensorboard callback to the callbacks list.
...
def callback_checkpoint(config):
    ...
    if config["clb_checkpoint"]["path"] is None:
        os.makedirs(os.path.join(os.getcwd(), "model_checkpoints"), exist_ok=True)
