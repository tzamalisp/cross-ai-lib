...
class BaseModel:
    def __init__(self, config, model_name=None, callbacks=False):
        ...
        Args:
            ...
            model_name (str or None, optional): The model name. If not defined, current datetime is used
        ...
        # self.path_to_save_model = Path(project_configuration["project_directories"]["exports"]).joinpath("models")
        self.path_to_save_model = Path(project_configuration["project_store_path"]).joinpath("tf")

        if not self.path_to_save_model.exists():
            logging.debug("Creating directory for saving model.".
                          format(self.path_to_save_model.relative_to(project_configuration["project_store_path"])))
        ...
        # log_dir is used by tensorboard to log variables related to the execution of the NN.
        log_dir = self.path_to_save_model
        self.path_to_tf_logs = log_dir

        logging.debug("Path to TF logs (usage with tensorboard) : {}".format(self.path_to_tf_logs))

        self.callbacks = callbacks
        self.optimizer = None
        if config.get("optimizer"):
            self.optimizer = config["optimizer"]
            HP_OPTIMIZER = hp.HParam("optimizer", hp.Discrete([config["optimizer"]]))
            self.hparams[HP_OPTIMIZER] = config["optimizer"]
        self.learning_rate = None
        if config.get("learning_rate"):
            self.learning_rate = config["learning_rate"]
            HP_LEARNING_RATE = hp.HParam("learning_rate", hp.Discrete([config["learning_rate"]]))
            self.hparams[HP_LEARNING_RATE] = HP_LEARNING_RATE.domain.values[0]
        self.epochs = config.get("epochs", 0)
        self.batch_size = None
        if config.get("batch_size"):
            self.batch_size = config["batch_size"]
            HP_BATCH_SIZE = hp.HParam("batch_size", hp.Discrete([config["batch_size"]]))
            self.hparams[HP_BATCH_SIZE] = config["batch_size"]
        self.nn_metrics = nn_default_parameters["nn_metrics"] if "nn_metrics" not in config.keys() \
            else config["nn_metrics"]
        self.nn_loss_function = nn_default_parameters["nn_loss_function"] if "nn_loss_function" not in config.keys() \
            else config["nn_loss_function"]

        # Initialize a model
        self.model = None
        self.iters_per_epoch = None
        self.mcdropout = False  # Default predictions behaviour. By default the MonteCarlo predictions is deactivated.
        self.mc_iterations = 1
        # Initialize NN architecture parameters. Set by the calling of define model for each model arch.
        self.arch = Bunch()
        ...
        # history would be stored (from tensorflow.fit function). This additional
        # dictionary is used because history from fit is writting data per epoch.
        # Thus in case fit iscalled again (e.g. in case of k-fold, only the last
    ...
    def callbacks_init(self):
        ...
        if calbacks_configuration["tensorboard"]["active"]:
            tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=self.path_to_tf_logs)
    ...
    def compile_model(self, optimizer=None, learning_rate=None, loss=None, nn_metrics=None):
        """
    ...
    def fit(self, train_dataset, validation_dataset, verbose=None):
        ...
        self.history = self.model.fit(x=np.vstack((train_dataset[0], validation_dataset[0])),
    ...
    def plot_performance(self, save=False):
        ...
        plot_model_performance(self.history, path_to_save=path_to_save_mp if save else None)
    ...
    def generate_classification_report(self, test_y, predictions, output_dict=True):
        """
        ...
        if len(predictions.shape) > 1:
            predictions = np.argmax(predictions, axis=1)
        results = classification_report(test_y, predictions, output_dict=output_dict)
        if output_dict:
            logging.info(
