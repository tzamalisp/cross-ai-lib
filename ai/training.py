...
# cross ai imports
from tools.ai.nn_xception_time import xception_time
from tools.ai.nn_inception_time_func import inception_time
from tools.ai.nn_bi_lstm import bi_lstm
from tools.ai.callbacks_functions import callbacks_list
from tools.ai.argument_checker import model_arguments_check_transform
from sklearn.model_selection import ShuffleSplit


def train_keras_clf(config, train_data, labels, model_name="xception_time", mode="randomized"):
    """
    ...
    print("- Type of kernel constraint:", type(param_grid["kernel_constraint"][0]))
    print("- Type of kernel regularize:", type(param_grid["kernel_regularize"][0]))
    ...
    if config["fit_grid_params"]["cv"] == 1:
        config["fit_grid_params"]["cv"] = ShuffleSplit(test_size=0.20, n_splits=1, random_state=None)
    ...
    callbacks_lst = callbacks_list(config=config, lr_scheduler=True, checkpoint=False)
...
def load_tf_clf(clf_params, grid_params, model_selection="xception_time"):
    ...
    if model_selection == "xception_time":
        model = xception_time(number_of_classes=clf_params["number_of_classes"],
        ...
        model = inception_time(number_of_classes=clf_params["number_of_classes"],
                               train_data_shape=clf_params["train_data_shape"],
                               kernel_initialize=grid_params["kernel_initialize"],
                               kernel_regularize=grid_params["kernel_regularize"],
                               kernel_constraint=grid_params["kernel_constraint"],
    ...
    print(model.summary())
    keras.utils.plot_model(model, to_file="{}_final.png".format(model_selection),
                           show_shapes=True,
...
def print_default_nn_models(config, model_name="xception_time"):
    ...
    """
    if model_name == "xception_time":
        model = xception_time(number_of_classes=config["clf_params"]["number_of_classes"],
                              train_data_shape=config["clf_params"]["train_data_shape"])
    elif model_name == "inception_time":
        model = inception_time(number_of_classes=config["clf_params"]["number_of_classes"],
                               train_data_shape=config["clf_params"]["train_data_shape"])
    elif model_name == "bi_lstm":
        model = bi_lstm(number_of_classes=config["clf_params"]["number_of_classes"],
                        train_data_shape=config["clf_params"]["train_data_shape"])
    else:
